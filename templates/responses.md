...
* If it is not prediction market but card deposit  
  * **If user says "deposit delayed"**: First ask which chain they deposited on.  
    * Options: **Hyperliquid Core**, **Arbitrum**, **Polygon**, **Solana**.  
    * Context: Hyperliquid Core deposits are typically instantaneous; Arbitrum, Polygon, and Solana share one infrastructure; Hyperliquid Core runs on a separate system.  
    * Response:  
      Hi, which chain did you deposit on? We support Hyperliquid Core, Arbitrum, Polygon, and Solana. This helps us narrow down where to look.  
